...
const unsigned long pumpMaxTimer = 10000;
...
void loop() {
  switch (state) {
    ...
    case MeasurePat_vent:
      if (accumulatedMillis > ventTimer - 2.0) {
        timerMillis = millis();
    ...
    case Admin_P:
      ...
      break;
    case Admin_A:
      if (accumulatedMillis > ventTimer - 2.0 || resP + atP < atP) {
        timerMillis = millis();
        state = Admin_A_2s;
      }
      break;
    ...
    case Admin_B:
      if (accumulatedMillis > ventTimer - 2.0 || cellP + atP < atP) {
        timerMillis = millis();
        state = Admin_B_2s;
      }
      break;
    ...
    case Running:
      if (accumulatedMillis > runningTimer){
        ...
        if (cellP - atP < pCellTarget - 200) {
          prop_int_nxt = 400;
        } else if (cellP - atP < pCellTarget - 50) {
          prop_int_nxt = 300;
        } else {
          prop_int_nxt = 0;
        }
